...
std::vector<float> GemmCUBLAS(
    ...
    cudaMemcpyAsync(result.data(), device_result, matrix_size, cudaMemcpyDeviceToHost, stream);
    if (error != cudaSuccess) {
        cudaFree(device_input_lhs);
        cudaFree(device_input_rhs);
        cudaFree(device_result);
...
int main() {
    ...
    GemmCUBLAS(input_lhs, input_rhs, MATRIX_SHAPE); // warming up
    ...
    for (int experiment_id = 0; experiment_id < NUM_EXPERIMENTS; ++experiment_id) {
        ...
        const auto result = GemmCUBLAS(input_lhs, input_rhs, MATRIX_SHAPE);

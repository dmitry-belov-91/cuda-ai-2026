...
def layernorm_pycuda(input, gamma, beta, row_size, eps=1e-5):
    ...
    x = cp.asarray(input, dtype=cp.float32)
    g = cp.asarray(gamma, dtype=cp.float32)
    b = cp.asarray(beta, dtype=cp.float32)
    y = cp.empty_like(x)

    threads = 1024
    blocks = x.size // row_size
    
    layernorm_kernel((blocks,), (threads,), (x, g, b, y, cp.int32(row_size), cp.float32(eps)))
